...
use crate::ports::{OrgChartStateRecord, OrgToolExecutionOutput, OrgToolStore};
...
fn invalid_input(message: impl Into<String>) -> DomainError {
...
fn parse_requests(args: &Value) -> Result<Vec<OrgV2Request>, DomainError> {
...
fn upsert_agent_manifest(manifests: &mut Vec<Value>, operator: &Operator) -> Result<(), DomainError> {
...
fn read_business_unit_item(
    ...
    item: &Value,
    client_op_id: Option<String>,
) -> Result<(OperationResult, Value), DomainError> {
...
) -> Result<(Vec<OperationResult>, Vec<Value>), DomainError> {
...
pub fn build_org_compact_preload(store: &dyn OrgToolStore) -> DomainResult<String> {
        let existing_state = store.load_org_chart_state()?;
...
pub fn execute_org_manage_entities_v2(
        ...
        let mut manifests = store.list_agent_manifests()?;
        ...
        state.snapshot = serde_json::to_value(snapshot)
            .map_err(|error| DomainError::Internal(error.to_string()))?;
        store.save_org_chart_state(&state)?;
        store.replace_agent_manifests(&manifests)?;
